...
def train(
    ...
    **kwargs,
    ...
    # Dataset is in the parent directory (mlp-image-classification/)
    data_path = Path(__file__).parent.parent / "classification_data"
    train_data = load_data(str(data_path / "train"), shuffle=True, batch_size=batch_size, num_workers=0)
    val_data = load_data(str(data_path / "val"), shuffle=False)
    ...
    # save model in the src directory
    save_model(model)
